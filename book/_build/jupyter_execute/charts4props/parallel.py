...
# **パラレルセットグラフ（Parallel Set Graph）** とは，複数の質的変数に対して，それらの比率を **平行棒の面積** で表現するグラフです．
# 三つ以上の質的変数に対しても適用可能であるという利点があります．
# パラレルセットグラフを作成する上でのポイントは，[最も強調したい質的変数を左側に配置し，かつ色付けすること](https://clauswilke.com/dataviz/nested-proportions.html#parallel-sets)です．
...
# 
# :::{note}
# 上記は`datePublished`に基づく分析結果ですが，私が調べた限り，これを裏付けるような情報はありませんでした．
# 私が`datePublished`の解釈を誤っている可能性がありますので，ご注意ください．
# :::
